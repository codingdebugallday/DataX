...
public class ReaderModelParser {
    ...
    public static OTSColumn parseOTSColumn(Map<String, Object> item) {
        ...
        } else if (item.containsKey(OTSConst.NAME) && item.containsKey(OTSConst.DECODE) && item.size() == 2) {
            Object name = item.get(OTSConst.NAME);
            Object decode = item.get(OTSConst.DECODE);
            if (name instanceof String && decode instanceof String) {
                String nameStr = (String) name;
                String decodeStr = (String) decode;
                return OTSColumn.fromDecodeColumn(nameStr, decodeStr);
            } else {
                throw new IllegalArgumentException("Invalid 'column', Can not parse map to 'OTSColumn', the value is not a string.");
            }
        } else if (item.containsKey(OTSConst.NAME) && item.containsKey(OTSConst.ENCODE) && item.size() == 2) {
            Object name = item.get(OTSConst.NAME);
            Object encode = item.get(OTSConst.ENCODE);
            if (name instanceof String && encode instanceof String) {
                String nameStr = (String) name;
                String encodeStr = (String) encode;
                return OTSColumn.fromEncodeColumn(nameStr, encodeStr);
            } else {
                throw new IllegalArgumentException("Invalid 'column', Can not parse map to 'OTSColumn', the value is not a string.");
            }
        } else {
