...
public class DFSUtil {
    private static final Logger LOG = LoggerFactory.getLogger(HdfsPlusReader.Job.class);
    ...
    private void kerberosAuthentication(String kerberosPrincipal, String kerberosKeytabFilePath) {
        if (haveKerberos && StringUtils.isNotBlank(this.kerberosPrincipal) && StringUtils.isNotBlank(this.kerberosKeytabFilePath)) {
            ...
            try {
                ...
                throw DataXException.asDataXException(HdfsPlusReaderErrorCode.KERBEROS_LOGIN_ERROR, message, e);
            }
        }
    }
    ...
    public void mkdir(String hdfsPath) {
        try {
            ...
            String message = String.format("无法创建目录[%s],请确认您的配置项fs.defaultFS是否正确，" +
                    "是否有读写权限，网络是否已断开！", hdfsPath);
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusReaderErrorCode.PATH_CONFIG_ERROR, e);
        }
    }
    ...
    private void addSourceFileByType(String filePath) {
        ...
        if (isMatchedFileType) {
            ...
            throw DataXException.asDataXException(
                    HdfsPlusReaderErrorCode.FILE_TYPE_UNSUPPORT, message);
        }
    }
    ...
    public InputStream getInputStream(String filepath) {
        ...
        try {
            ...
            throw DataXException.asDataXException(HdfsPlusReaderErrorCode.READ_FILE_ERROR, message, e);
        }
    }
    ...
    public void sequenceFileStartRead(String sourceSequenceFilePath, Configuration readerSliceConfig,
        ...
        try {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusReaderErrorCode.READ_SEQUENCEFILE_ERROR, message, e);
        } finally {
    ...
    public void rcFileStartRead(String sourceRcFilePath, Configuration readerSliceConfig,
        ...
        try {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusReaderErrorCode.READ_RCFILE_ERROR, message, e);
        } finally {
            try {
        ...
        if (columnIndexMax >= 0) {
            ...
            try {
                ...
                String message = String.format("从orcfile文件路径[%s]中读取数据发生异常，请联系系统管理员。"
                        , sourceOrcFilePath);
                LOG.error(message);
                throw DataXException.asDataXException(HdfsPlusReaderErrorCode.READ_FILE_ERROR, message);
            }
        } else {
            String message = String.format("请确认您所读取的列配置正确！columnIndexMax 小于0,column:%s", JSON.toJSONString(column));
            throw DataXException.asDataXException(HdfsPlusReaderErrorCode.BAD_CONFIG_VALUE, message);
        }
    }
    ...
    private int getMaxIndex(List<ColumnEntry> columnConfigs) {
        ...
        for (ColumnEntry columnConfig : columnConfigs) {
            ...
            if (columnIndex != null && columnIndex < 0) {
                ...
                LOG.error(message);
                throw DataXException.asDataXException(HdfsPlusReaderErrorCode.CONFIG_INVALID_EXCEPTION, message);
