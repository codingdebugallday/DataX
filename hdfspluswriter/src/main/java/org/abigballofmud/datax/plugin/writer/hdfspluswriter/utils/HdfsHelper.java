package org.abigballofmud.datax.plugin.writer.hdfspluswriter.utils;
...
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.HdfsPlusWriter;
...
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.enums.SupportHiveDataType;
...
public class HdfsHelper {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(HdfsPlusWriter.Job.class);
    ...
    public void getFileSystem(String defaultFS, Configuration taskConfig) {
        ...
        try {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.CONNECT_HDFS_IO_ERROR, e);
        } catch (Exception e) {
        ...
        if (null == fileSystem) {
            String message = String.format("获取FileSystem失败,请检查HDFS地址是否正确: [%s]",
                    "message:defaultFS =" + defaultFS);
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.CONNECT_HDFS_IO_ERROR, message);
        }
    }
    ...
    private void kerberosAuthentication(String kerberosPrincipal, String kerberosKeytabFilePath) {
        if (haveKerberos && StringUtils.isNotBlank(this.kerberosPrincipal) && StringUtils.isNotBlank(this.kerberosKeytabFilePath)) {
            ...
            try {
                ...
                LOG.error(message);
                throw DataXException.asDataXException(HdfsPlusWriterErrorCode.KERBEROS_LOGIN_ERROR, e);
            }
        }
    }
    ...
    public void renameFile(HashSet<String> tmpFiles, HashSet<String> endFiles) {
        ...
        if (tmpFiles.size() != endFiles.size()) {
            ...
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.HDFS_RENAME_FILE_ERROR, message);
        } else {
            try {
    ...
    public FSDataOutputStream getOutputStream(String path) {
        ...
        try {
            ...
            String message = String.format("Create an FSDataOutputStream at the indicated Path[%s] failed: [%s]",
                    "message:path =" + path);
            LOG.error(message);
            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.WRITE_FILE_IO_ERROR, e);
        }
            ...
            switch (columnType) {
                ...
                default:
                    throw DataXException
                            .asDataXException(
                                    HdfsPlusWriterErrorCode.ILLEGAL_VALUE,
                                    String.format(
