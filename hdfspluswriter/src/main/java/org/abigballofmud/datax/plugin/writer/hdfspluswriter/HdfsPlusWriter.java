package org.abigballofmud.datax.plugin.writer.hdfspluswriter;
...
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.constants.Constant;
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.constants.Key;
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.enums.HdfsPlusWriterErrorCode;
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.enums.SupportHiveFileType;
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.utils.HdfsHelper;
import org.abigballofmud.datax.plugin.writer.hdfspluswriter.utils.ShellUtil;
 ...
 */
public class HdfsPlusWriter extends Writer {
    ...
    public static class Job extends Writer.Job {
        ...
        private void validateParameter() {
            this.defaultFS = this.writerSliceConfig.getNecessaryValue(Key.DEFAULT_FS, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            // fileType check
            this.fileType = this.writerSliceConfig.getNecessaryValue(Key.FILE_TYPE, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            if (!EnumUtils.isValidEnum(SupportHiveFileType.class, fileType.toUpperCase())) {
                String message = "HdfsWriter插件目前只支持ORC和TEXT两种格式的文件,请将filetype选项的值配置为ORC或者TEXT";
                throw DataXException.asDataXException(HdfsPlusWriterErrorCode.ILLEGAL_VALUE, message);
            }
            // path
            this.path = this.writerSliceConfig.getNecessaryValue(Key.PATH, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            if (!path.startsWith("/")) {
            ...
            // fileName
            this.fileName = this.writerSliceConfig.getNecessaryValue(Key.FILE_NAME, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            // columns check
            this.columns = this.writerSliceConfig.getListConfiguration(Key.COLUMN);
            if (null == columns || columns.size() == 0) {
                throw DataXException.asDataXException(HdfsPlusWriterErrorCode.REQUIRED_VALUE, "您需要指定 columns");
            } else {
                for (Configuration eachColumnConf : columns) {
                    eachColumnConf.getNecessaryValue(Key.NAME, HdfsPlusWriterErrorCode.COLUMN_REQUIRED_VALUE);
                    eachColumnConf.getNecessaryValue(Key.TYPE, HdfsPlusWriterErrorCode.COLUMN_REQUIRED_VALUE);
                }
            }
            // writeMode check
            this.writeMode = this.writerSliceConfig.getNecessaryValue(Key.WRITE_MODE, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            writeMode = writeMode.toLowerCase().trim();
            Set<String> supportedWriteModes = Sets.newHashSet("append", "nonconflict");
            if (!supportedWriteModes.contains(writeMode)) {
                throw DataXException.asDataXException(HdfsPlusWriterErrorCode.ILLEGAL_VALUE,
            ...
            if (null == fieldDelimiter) {
                throw DataXException.asDataXException(HdfsPlusWriterErrorCode.REQUIRED_VALUE,
            ...
            if (haveKerberos) {
                this.writerSliceConfig.getNecessaryValue(Key.KERBEROS_KEYTAB_FILE_PATH, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
                this.writerSliceConfig.getNecessaryValue(Key.KERBEROS_PRINCIPAL, HdfsPlusWriterErrorCode.REQUIRED_VALUE);
            }
            // encoding check
            ...
            // preSql, postSql check
            preSqlList = this.writerSliceConfig.getList(Key.PRE_SQL, String.class);
            if (!CollectionUtils.isEmpty(preSqlList)) {
                this.writerSliceConfig.set(Key.PRE_SQL, preSqlList);
            }
        ...
        public void prepare() {
            ...
            if (hdfsHelper.isPathExists(path)) {
                ...
                // preSqlList
                if (preSqlList != null) {
                    LOG.info("reader prepare() presql begin...");
                    for (String preSql : preSqlList) {
                        try {
                            ShellUtil.exec(new String[]{"hive", "-e", Constant.DOUBLE_QUOTATION + preSql + Constant.DOUBLE_QUOTATION});
                        } catch (Exception e) {
                            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.HIVE_SQL_EXEC_ERROR,
                                    String.format("您配置的preSql: [%s] 有错, 请检查sql是否正确.", preSql));
                        }
                    }
                }
            } else {
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            for (int i = 0; i < mandatoryNumber; i++) {
                ...
                fullFileName = String.format(Constant.FULL_FILE_PATH, defaultFS, storePath, filePrefix, fileSuffix);
                endFullFileName = String.format(Constant.FULL_FILE_PATH, defaultFS, endStorePath, filePrefix, fileSuffix);
        ...
        private String buildTmpFilePath(String userPath) {
            ...
            if (!isEndWithSeparator) {
                tmpFilePath = String.format(Constant.TMP_FILE_PATH, userPath, tmpSuffix, IOUtils.DIR_SEPARATOR);
            } else if ("/".equals(userPath)) {
                tmpFilePath = String.format(Constant.TMP_FILE_PATH, userPath, tmpSuffix, IOUtils.DIR_SEPARATOR);
            } else {
                tmpFilePath = String.format(Constant.TMP_FILE_PATH, userPath.substring(0, userPath.length() - 1), tmpSuffix, IOUtils.DIR_SEPARATOR);
            }
    ...
    public static class Task extends Writer.Task {
        ...
        public void post() {
            // postSqlList
            LOG.info("writer post() postSql execute...");
            if (postSqlList != null) {
                for (String postSql : postSqlList) {
                    try {
                        if (!ShellUtil.exec(new String[]{"hive", "-e", Constant.DOUBLE_QUOTATION + postSql + Constant.DOUBLE_QUOTATION})) {
                            throw DataXException.asDataXException(HdfsPlusWriterErrorCode.HIVE_SQL_EXEC_ERROR,
                                    String.format("您配置的postSql: [%s] 有错, 请检查sql是否正确.", postSql));
                        }
                    } catch (Exception e) {
                        throw DataXException.asDataXException(HdfsPlusWriterErrorCode.HIVE_SQL_EXEC_ERROR,
                                String.format("您配置的postSql: [%s] 有错, 请检查sql是否正确.", postSql));
