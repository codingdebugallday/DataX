package com.alibaba.datax.plugin.reader.clickhouse;
...
public class ClickhouseReader extends Reader {
    ...
    public static class Job extends Reader.Job {
        ...
        public void init() {
            ...
            Integer fetchSize = this.originalConfig.getInt(Constant.FETCH_SIZE);
            if (fetchSize != null) {
                LOG.warn("对 clickhouse 不需要配置 fetchSize, clickhouse 将会忽略这项配置. 如果您不想再看到此警告,请去除fetchSize 配置.");
            }
            this.originalConfig.set(Constant.FETCH_SIZE, Integer.MIN_VALUE);
            this.commonRdbmsReaderJob = new CommonRdbmsReader.Job(DATABASE_TYPE);
            OriginalConfPretreatmentUtil.DATABASE_TYPE = DATABASE_TYPE;
            OriginalConfPretreatmentUtil.doPretreatmentWithUnnecessary(originalConfig);
        }
